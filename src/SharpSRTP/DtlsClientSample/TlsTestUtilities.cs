...
namespace Org.BouncyCastle.Tls.Tests
{
    public class TlsTestUtilities
    {
        ...
        internal static PemObject LoadPemResource(string resource)
        {
            ...
            {
                using (var p = new PemReader(new StreamReader(FindTestResource("tls", "credentials", key))))
                {
        ...
        internal static Stream FindTestResource(string path1, string path2, string path3) =>
            File.OpenRead(Path.Combine(Directory.GetCurrentDirectory(), path1, path2, path3));
